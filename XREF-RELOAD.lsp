...
;;;
;;; Commands:
;;;   XRL        — Reload xrefs using current configuration
;;;   XRLCONFIG  — Configure XRL settings
;;;                 M = mode  (Quick / Select / Saved)
;;;                 S = saved set  (Add / Remove / Clear)
;;;                 A = SaveAll toggle  (On / Off)
;;;   XRL_VER    — Print version info
;;;
;;; Selection format: 1,3,5-8,12  |  * (all)  |  0 (cancel)
;;;
;;; ===========================================================

(setq *xrl-version* "2.1")

;;; Persistent globals — initialized once per session.
(if (not *xrl-mode*)    (setq *xrl-mode*    "Q"))
...
;;; Returns list of xref block names in the current drawing.
...
;;; Strips leading/trailing spaces using index pointers.
...
;;; Splits STR on single-char DELIM; returns list of substrings.
...
;;; Parses "1,3,5-8,12" into a deduplicated list of ints.
...
;;; Returns display name for a mode code.
...
;;; Displays a numbered list and returns the user's selection, or nil.
...
;;; ===========================================================
;;;   S A V E A L L   ( t a r g e t e d )
;;; ===========================================================

;;; QSAVE open, modified drawings whose base name is in TARGET-NAMES.
;;; Skips the active document and guards all property access against errors.
;;; Silent no-op when *xrl-saveall* is nil.
            ...
            (if (member (strcase basename) target-names)
              (progn
                ...
                (if (and (not (vl-catch-all-error-p modified))
                         (= modified :vlax-false))  ; Saved=:vlax-false means unsaved changes exist
                  (progn
                    (princ (strcat "\n  [SAVE] " basename))
...
(defun xrl:reload-and-report (names xlist / count xname)
  ...
  (princ (strcat "\n\n  " (itoa count) " of "
                 (itoa (length names)) " xref(s) reloaded."))
  count
)
...
(defun xrl:quick-mode ( / xlist)
  ...
  (if xlist
    (progn
      (xrl:saveall-targets xlist)
      (xrl:reload-and-report xlist xlist)
    )
    (princ "\n  No xrefs found.")
  )
)
...
(defun xrl:select-mode ( / xlist picked)
  ...
  (if (null xlist)
    ...
    (progn
      ...
      (if picked
        (progn
          (xrl:saveall-targets picked)
          (xrl:reload-and-report picked xlist)
        )
        (princ "\n  Cancelled.")
      )
    )
...
(defun xrl:saved-mode ( / xlist)
  ...
  (cond
    ...
    ((null *xrl-saved*) (princ "\n  Saved set is empty. Use XRLCONFIG > S > Add."))
    (T
...
;;; Prints the XRLCONFIG status panel, including saved set contents.
(defun xrlconfig:print-status ( / xname)
  (princ (strcat
    "\n  ┌─── XRL Config ──────────────────────────"
    "\n  │  Mode:    " (xrl:mode-name *xrl-mode*)
...
(defun c:XRLCONFIG ( / *error* old-cmdecho old-filedia input)
  ...
  ;;; Reprint status after any change so result is immediately visible.
  (xrlconfig:print-status)
